...
from graphrag.vector_stores.spanner import SpannerVectorStore, _safe_identifier
...
class TestSpannerVectorStore(unittest.TestCase):
    ...
    def test_similarity_search_sql_uses_backtick_quoted_identifiers(self):
        """SQL generated by similarity_search_by_vector must quote all identifiers."""
        mock_snapshot = MagicMock()
        self.mock_database.snapshot.return_value.__enter__.return_value = mock_snapshot
        mock_snapshot.execute_sql.return_value = []
        ...
        # Every field and the table name must appear backtick-quoted
        for identifier in (
            self.index_name,
            self.config.id_field,
            self.config.text_field,
            self.config.vector_field,
            self.config.attributes_field,
        ):
            self.assertIn(f"`{identifier}`", sql)

    def test_search_by_id_sql_uses_backtick_quoted_identifiers(self):
        """SQL generated by search_by_id must quote all identifiers."""
        mock_snapshot = MagicMock()
        self.mock_database.snapshot.return_value.__enter__.return_value = mock_snapshot
        mock_snapshot.execute_sql.return_value = []

        self.store.search_by_id("1")
